...
InternalData data = findTestData("postTLCarrierFreerunReroute")
InternalData freerun = findTestData("postTLCarrierFreerunReroute1")
...
for (def index : 0..data.getRowNumbers() - 1) {
	...
	for (def freerunIndex : 0..freerun.getRowNumbers() - 1) {
		...
		CustomKeywords.'com.SwisslogUtilities.postTranslogicCarrierEvent' freerun.internallyGetValue("Carrier", freerunIndex), freerun.internallyGetValue("EventType", freerunIndex), freerun.internallyGetValue("SourceStationId", freerunIndex), freerun.internallyGetValue("DestinationStationAddress", freerunIndex), freerun.internallyGetValue("DestinationStationId", freerunIndex), freerunIncrement, freerun.internallyGetValue("User", freerunIndex)
