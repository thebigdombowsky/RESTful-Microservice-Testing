...
for (def index : 0..registerCarrierData.getRowNumbers() - 1) {
	...
	CustomKeywords.'com.SwisslogUtilities.registerCarrier' registerCarrierData.internallyGetValue("Carrier", index), registerCarrierData.internallyGetValue("PrepStation", index), registerCarrierData.internallyGetValue("Identifier", index), transactionId, 'true'

		for (def eventsIndex : 0..data.getRowNumbers() - 1) {
	
		CustomKeywords.'com.SwisslogUtilities.postTranslogicCarrierEvent' data.internallyGetValue("Carrier", eventsIndex), data.internallyGetValue("EventType", eventsIndex), data.internallyGetValue("SourceStationId", eventsIndex), data.internallyGetValue("DestinationStationAddress", eventsIndex), data.internallyGetValue("DestinationStationId", eventsIndex), transactionId, data.internallyGetValue("User", eventsIndex)
