...
InternalData registerData = findTestData("registerTLCarriersData")
...
for (def index : 0..data.getRowNumbers() - 1) {
	...
	CustomKeywords.'com.SwisslogUtilities.registerCarrier' registerData.internallyGetValue("Carrier", index), registerData.internallyGetValue("Type", index), registerData.internallyGetValue("PrepStation", index), registerData.internallyGetValue("Identifier", index), 'true'

	println("Waiting for timeout to expire...")
	sleep(61000)
	
	CustomKeywords.'com.SwisslogUtilities.postTPNCarrierEvent' data.internallyGetValue("Carrier", index), data.internallyGetValue("EventType", index), data.internallyGetValue("SourceStationId", index), data.internallyGetValue("DestinationStationAddress", index), data.internallyGetValue("DestinationStationId", index), transactionId, data.internallyGetValue("User", index)
