...
InternalData data = findTestData("dispenseOrdersCharacterLimit")
	...
	def output = CustomKeywords.'com.SwisslogUtilities.createNewOrder' data.internallyGetValue("Quantity", index), data.internallyGetValue("Description", index), data.internallyGetValue("Identifier", index), data.internallyGetValue("Location", index)

	def destinationLocationId = output.destinationLocationId
	def orderLocationId = output.locationId

	CustomKeywords.'com.SwisslogUtilities.cancelOrder'output.locationId, output.destinationLocationId,
