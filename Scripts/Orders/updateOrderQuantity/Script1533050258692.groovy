...
WebUI.comment('Call the updateOrderQuantity Keyword to change the quantity of the order just created via REST endpoint api/dispenseorders')

InternalData data = findTestData("dispenseOrdersData")

	for (def index : 0..data.getRowNumbers() - 1) {
	
	def dispenseOrderResult = CustomKeywords.'com.SwisslogUtilities.createNewOrder' data.internallyGetValue("Quantity", index), data.internallyGetValue("Description", index), data.internallyGetValue("Identifier", index), data.internallyGetValue("Location", index)
	
	CustomKeywords.'com.SwisslogUtilities.updateOrderQuantity' '225', dispenseOrderResult.locationId
